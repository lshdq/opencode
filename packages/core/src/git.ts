...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const discover = Effect.fn("Git.repo.discover")(function* (input: AbsolutePath) {
      ...
      // `--show-toplevel` must be last: in a bare repository it fails (exit 128),
      // but git still flushes the preceding flags' output lines.
      const result = yield* git(["rev-parse", "--git-dir", "--git-common-dir", "--show-toplevel"])
      const [gitDir, commonDir, topLevel] = result.text.split("\n")
      if (!gitDir?.trim() || !commonDir?.trim()) return undefined

      return new Repository({
        worktree: AbsolutePath.make(topLevel?.trim() ? resolvePath(cwd, topLevel) : cwd),
