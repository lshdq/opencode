...
4. Global and project AGENTS.md files and configured instructions
...
Edit a global or upward-discovered `AGENTS.md` or a configured instruction source while a session is running to update its guidance.
...
Add files, glob patterns, or HTTP(S) URLs to `instructions` in `opencode.jsonc`. OpenCode loads them alongside discovered `AGENTS.md` files:
...
- Relative paths and globs are searched from the current Location upward to the project root; absolute patterns match at their specified location, and `~/` expands to the home directory.
- Configured sources are read in configuration order, with nearer matches before farther ones. Sources that resolve to the same file as an automatically discovered `AGENTS.md` are loaded only once.
